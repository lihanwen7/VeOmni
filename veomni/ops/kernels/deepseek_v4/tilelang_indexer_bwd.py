...
def indexer_bwd_interface(
    ...
    # The compiled kernel requires both a block_I multiple and a power of two.
    # Padding with invalid indices and zero gradients preserves the requested
    # top-k semantics while allowing arbitrary user-facing top-k values.
    padded_topk = max(32, 1 << (k_top - 1).bit_length())
