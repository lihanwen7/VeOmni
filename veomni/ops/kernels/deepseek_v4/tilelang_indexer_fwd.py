...
def indexer_fwd_interface(q, kv, weights, cu_seqlen_ks, cu_seqlen_ke, clean_logits=True):
    ...
    if padded_seq_len != seq_len:
        q = torch.cat([q, q.new_zeros(padded_seq_len - seq_len, heads, index_dim)], dim=0)
        weights = torch.cat([weights, weights.new_zeros(padded_seq_len - seq_len, heads)], dim=0)
        cu_seqlen_ks = torch.cat([cu_seqlen_ks, cu_seqlen_ks.new_full((padded_seq_len - seq_len,), seq_len_kv)])
        cu_seqlen_ke = torch.cat([cu_seqlen_ke, cu_seqlen_ke.new_full((padded_seq_len - seq_len,), seq_len_kv)])
    if padded_seq_len_kv != seq_len_kv:
        kv = torch.cat([kv, kv.new_zeros(padded_seq_len_kv - seq_len_kv, index_dim)], dim=0)

    clean_logits_kernel = clean_logits_()
    tl_indexer_fwd_kernel = tl_indexer_fwd_impl(heads=heads, index_dim=index_dim, block_N=block_n, block_Q=block_q)

    logits = torch.empty([padded_seq_len, padded_seq_len_kv], device=q.device, dtype=torch.float32)
    tl_indexer_fwd_kernel(
        q.contiguous().view(padded_seq_len * heads, index_dim),
        kv,
        logits,
    ...
    return logits[:seq_len, :seq_len_kv].contiguous()
