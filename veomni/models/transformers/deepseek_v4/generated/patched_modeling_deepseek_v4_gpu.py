...
#      Return top-k indices needed by the opt-in DSA attention backend
#    - method_override: DeepseekV4Attention.forward
#      Use FlashMLA sparse prefill forward with cuDNN FE DSA backward for CSA layers
...
class DeepseekV4CSACompressor(nn.Module):
    ...
    def forward(
        ...
        if top_k_indices.shape[-1] < self.indexer.index_topk:
            pad = top_k_indices.new_full(
                (*top_k_indices.shape[:2], self.indexer.index_topk - top_k_indices.shape[-1]), -1
            )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.compressor is not None:
            compressor_result = self.compressor(
